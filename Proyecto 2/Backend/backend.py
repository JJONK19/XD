...
RADIUS = 0.5  # Radius of the anemometer in meters
PULSES_PER_REVOLUTION = 20  # Number of pulses per revolution
MEASURE_INTERVAL = 1  # Time interval for measuring wind speed (in seconds)
...
def read_air_quality():
    ...
    while True:
        try:
            ...
            while True:
                with data_lock:
                    air_quality = max(chan.value*5/3+6000,0)  # Convert to ppm, adjust as necessary
                time.sleep(2)
        except Exception as e:
...
def write_to_csv():
    ...
    fieldnames = ["Timestamp", "BMP280 Pressure (hPa)", "AHT10 Temperature (°C)", "AHT10 Humidity (%)", "Wind Speed (m/s)",  "Air Quality (ppm)","Cloudy/Sunny"]
