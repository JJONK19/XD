...
_start:
    ...
    ldr x24, =frequencies   // Puntero al arreglo de frecuencias
...
parse_number:
    ...
    lsl x7, x21, #2         // Multiplicar el número por 4 para obtener el offset
    add x7, x24, x7         // Añadir el offset a la dirección base de frequencies
    ldr w8, [x7]            // Cargar la frecuencia actual
    add w8, w8, #1          // Incrementar la frecuencia
    str w8, [x7]            // Guardar la nueva frecuencia

    // Actualizar la moda si es necesario
    cmp w8, w25             // Comparar con la frecuencia más alta actual
    ble not_new_mode        // Si no es mayor, no actualizar la moda
    mov w25, w8             // Actualizar la frecuencia más alta
    mov x26, x21            // Actualizar el valor de la moda
...
parse_end:
    ...
    // Obtener y mostrar el primer valor
    ldr w1, [x28]  // Cargar el primer valor del arreglo
    ldr x0, =first_value_msg
    bl printf

    // Obtener y mostrar el último valor
    sub x2, x27, #1  // Índice del último elemento (total - 1)
    ldr w1, [x28, x2, lsl #2]  // Cargar el último valor del arreglo
